...
class ChartingState extends MusicBeatState
{
	...
	var noteTypes:Array<String> = ['Normal', 'Warning', 'Mystery Notes', 'Kill Notes', 'Static Notes', 'Phantom Notes', 'Signs Notes', 'No Animation'];
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSection].sectionNotes)
		{
			...
			if (daType != 0)
			{
				...
				typeText.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				if (daType == 5)
					{
						typeText.text = "5";
					}
	...
	function delleteNote(note:Note):Void
	{
		for (i in _song.notes[curSection].sectionNotes)
		{
			if (i[0] == note.strumTime && i[1] % (Main.ammo[_song.mania]) == note.noteData)
			{
				FlxG.log.add('FOUND EVIL NUMBER');
				_song.notes[curSection].sectionNotes.remove(i);
			}
	...
	private function addNote(?n:Note):Void
	{
		...
		if(noteData > -1) {
			...
			_song.notes[curSection].sectionNotes.push([noteStrum, noteData, noteType, event, text1, text2]);
		}
		curSelectedNote = _song.notes[curSection].sectionNotes[_song.notes[curSection].sectionNotes.length - 1];

		if (FlxG.keys.pressed.CONTROL && noteData > -1)
		{
			_song.notes[curSection].sectionNotes.push([noteStrum, (noteData + Main.ammo[_song.mania]) % (Main.ammo[_song.mania] * 2), noteSus, noteType]);
		}
