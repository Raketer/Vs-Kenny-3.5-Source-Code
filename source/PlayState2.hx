	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'pizza':
			{
					defaultCamZoom = 0.85;
					curStage = 'sas';
					roof = new FlxSprite(-600, -200).loadGraphic(Paths.image('weeb/techo'));
					roof.antialiasing = true;
					roof.scrollFactor.set(0.95, 0.95);
					roof.active = false;
					add(roof);
					...
					roofRed = new FlxSprite(-600, -200).loadGraphic(Paths.image('weeb/techoRed'));
					roofRed.antialiasing = true;
					roofRed.scrollFactor.set(0.95, 0.95);
					roofRed.active = false;
						//add(roofRed);

					lluvia = new FlxSprite(-600, -2250);
			...
			case 'southpurgation':
			{
					...
					skyZ.antialiasing = true;
					skyZ.scrollFactor.set(0.95, 0.95);
					skyZ.active = false;
					add(skyZ);

					var moutainsZ:FlxSprite = new FlxSprite(-800, -260).loadGraphic(Paths.image('weeb/moutainsKNZ'));
					moutainsZ.setGraphicSize(Std.int(moutainsZ.width * 1.1));
					moutainsZ.updateHitbox();
					moutainsZ.antialiasing = true;
					moutainsZ.scrollFactor.set(0.9, 0.9);
					moutainsZ.active = false;
					add(moutainsZ);

					var graveyardZ:FlxSprite = new FlxSprite(-820, -320).loadGraphic(Paths.image('weeb/cementerioKNZ'));
					graveyardZ.setGraphicSize(Std.int(graveyardZ.width * 1.2));
					graveyardZ.updateHitbox();
					graveyardZ.antialiasing = true;
		...
		if (curStage == 'sos')
		{
			var nieblaZ:FlxSprite = new FlxSprite(-550, 780);
			nieblaZ.frames = Paths.getSparrowAtlas('weeb/niebla');
			nieblaZ.setGraphicSize(Std.int(nieblaZ.width * 0.9));
			nieblaZ.animation.addByPrefix('idle', "humo 2", 24);
			nieblaZ.animation.play('idle');
			nieblaZ.scrollFactor.set(1.5, 1.5);
			nieblaZ.alpha = 0.3;
			add(nieblaZ);
		}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			switch(note.noteType) {
				...
				case 'Mysterio Note': //Mysterio Note
					if(cpuControlled) return;
					noteMiss(note.noteData);
					--songMisses;
					note.wasGoodHit = true;
